...
var SaveTenant = function () {
    ...
    if (Image2.files.length > 0) {
        for (var i = 0; i < Image2.files.length; i++) {
            $formData.append('file2-' + i, Image2.files[i]);
        }
    }

    $formData = new FormData();
    var Image2 = document.getElementById('file3');
    if (Image2.files.length > 0) {
        for (var i = 0; i < Image2.files.length; i++) {
            $formData.append('file3-' + i, Image2.files[i]);
        }
    }
    ...
    var photo = $("#file1").val();
    var adharcard = $("#file3").val();
    var policedocumentverification = $("#file2").val();
...
var TenantDetail = function (Id) {
    ...
    $.ajax({
        ...
        success: function (response) {
            ...
            html += "</br>";
            html += "<div class='col-sm-6'>";
            html += "<b>PoliceDocumentVerification:</b>&nbsp&nbsp&nbsp<span><img src='../Content/Img/" + response.model.policedocumentverification + "'style='width:150px;height:180px;'/> </span > ";
            html += "</br>";
